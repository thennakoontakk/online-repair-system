  ...
  useEffect(() => {
    ...
    onValue(requestsRef, (snapshot) => {
      ...
      if (requestsData) {
        ...
        setCompletedRequestsCount(requestsArray.filter(req => req.status === 'Completed').length);
    ...
    onValue(usersRef, (snapshot) => {
      ...
      if (usersData) {
        ...
        const roleData = usersArray.reduce((acc, user) => {
          acc[user.role] = (acc[user.role] || 0) + 1;
          return acc;
        }, {});
        setUsersByRole(Object.keys(roleData).map(role => ({ role, count: roleData[role] })));
      }
    });
  }, [database]);
        ...
        <div className="chart-container">
          ...
          <ResponsiveContainer width="100%" height={300}>
            <BarChart data={usersByRole}>
              ...
              <Tooltip />
              <Legend />
              <Bar dataKey="count" fill="#a4de6c" name="Users" />
            </BarChart>
          </ResponsiveContainer>
        </div>
      </section>
